...
##' @param smoothingMethod one of ("avg", "median", "min", "max", "mode", "none")
##' @return list:
...
##'   engineEdgeExperimental(start_network=start_network,
...
##'                          smoothingMethod = "mode"))
##' }

engineEdgeExperimental <- function(start_network,inputcoeff,ns,
  ...
  for(ncount in 1:ns){
    ...
    ## Section for switching different alternatives to average based smoothing
    if(smoothingMethod == "avg") {
      ## for(i in 1:repfac){
      ##   smmpleMat <- smmpleMat + inputmpleMat[(((i-1)*nedges+1):(i*nedges)),]
      ## }
      ## smmpleMat <- smmpleMat/repfac
      tmpmat <- matrix(NA, nrow = repfac,
                       ncol = length(c(inputmpleMat[1:nedges,])))
      for(i in 1:repfac) {
        tmpmat[i, ] <- c(inputmpleMat[(((i-1)*nedges+1):(i*nedges)),])
      }
      tmpmed <- apply(tmpmat, 2, mean)
      smmpleMat <- matrix(tmpmed, nrow = nedges)
    } else if(smoothingMethod == "median" ) {
      tmpmat <- matrix(NA, nrow = repfac,
                       ncol = length(c(inputmpleMat[1:nedges,])))
      for(i in 1:repfac) {
        tmpmat[i, ] <- c(inputmpleMat[(((i-1)*nedges+1):(i*nedges)),])
      }
      tmpmed <- apply(tmpmat, 2, stats::median)
      smmpleMat <- matrix(tmpmed, nrow = nedges)
    } else if(smoothingMethod == "min") {
      tmpmat <- matrix(NA, nrow = repfac,
                       ncol = length(c(inputmpleMat[1:nedges,])))
      for(i in 1:repfac) {
        tmpmat[i, ] <- c(inputmpleMat[(((i-1)*nedges+1):(i*nedges)),])
      }
      tmpmed <- apply(tmpmat, 2, min)
      smmpleMat <- matrix(tmpmed, nrow = nedges)
    } else if(smoothingMethod == "max") {
      tmpmat <- matrix(NA, nrow = repfac,
                       ncol = length(c(inputmpleMat[1:nedges,])))
      for(i in 1:repfac) {
        tmpmat[i, ] <- c(inputmpleMat[(((i-1)*nedges+1):(i*nedges)),])
      }
      tmpmed <- apply(tmpmat, 2, max)
      smmpleMat <- matrix(tmpmed, nrow = nedges)        
    } else if(smoothingMethod == "mode") {
      kdeMode <- function(foo){
        bar <- stats::density(foo)
        bar$x[which.max(bar$y)]
      }
      tmpmat <- matrix(NA, nrow = repfac,
                       ncol = length(c(inputmpleMat[1:nedges,])))
      for(i in 1:repfac) {
